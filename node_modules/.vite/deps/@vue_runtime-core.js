...
import { isString, isFunction as isFunction2, isPromise, isArray as isArray2, NOOP as NOOP2, getGlobalThis, extend as extend2, EMPTY_OBJ, toHandlerKey, toNumber, hyphenate, camelize, isOn, hasOwn as hasOwn2, isModelListener, hasChanged as hasChanged2, remove, isObject as isObject2, isSet, isMap as isMap2, isPlainObject, invokeArrayFns, isBuiltInDirective, capitalize as capitalize2, isGloballyWhitelisted, def as def2, isReservedProp, EMPTY_ARR, toRawType as toRawType2, makeMap as makeMap2, NO, normalizeClass, normalizeStyle } from "@vue/shared";
import { camelize as camelize2, capitalize as capitalize3, normalizeClass as normalizeClass2, normalizeProps, normalizeStyle as normalizeStyle2, toDisplayString, toHandlerKey as toHandlerKey2 } from "@vue/shared";
var stack = [];
...
function isEmitListener(options, key) {
  ...
  return hasOwn2(options, key[0].toLowerCase() + key.slice(1)) || hasOwn2(options, hyphenate(key)) || hasOwn2(options, key);
}
...
var PublicInstanceProxyHandlers = {
  get({ _: instance }, key) {
    ...
    if (setupState !== EMPTY_OBJ && setupState.__isScriptSetup && hasOwn2(setupState, key)) {
    ...
    if (key[0] !== "$") {
      ...
      if (n !== void 0) {
      ...
      } else if (setupState !== EMPTY_OBJ && hasOwn2(setupState, key)) {
        accessCache[key] = 1;
        return setupState[key];
      } else if (data !== EMPTY_OBJ && hasOwn2(data, key)) {
        accessCache[key] = 2;
        return data[key];
      } else if (
        ...
        accessCache[key] = 3;
        return props[key];
      } else if (ctx !== EMPTY_OBJ && hasOwn2(ctx, key)) {
        accessCache[key] = 4;
        return ctx[key];
    ...
    key.indexOf("__v") !== 0)) {
      if (data !== EMPTY_OBJ && isReservedPrefix(key[0]) && hasOwn2(data, key)) {
  ...
  set({ _: instance }, key, value) {
    ...
    if (setupState !== EMPTY_OBJ && hasOwn2(setupState, key)) {
  ...
  has({ _: { data, setupState, accessCache, ctx, appContext, propsOptions } }, key) {
    let normalizedProps;
    return !!accessCache[key] || data !== EMPTY_OBJ && hasOwn2(data, key) || setupState !== EMPTY_OBJ && hasOwn2(setupState, key) || (normalizedProps = propsOptions[0]) && hasOwn2(normalizedProps, key) || hasOwn2(ctx, key) || hasOwn2(publicPropertiesMap, key) || hasOwn2(appContext.config.globalProperties, key);
  },
...
function applyOptions(instance) {
  ...
  if (render && instance.render === NOOP2) {
...
function updateProps(instance, rawProps, rawPrevProps, optimized) {
  ...
  if (
    ...
    for (const key in rawCurrentProps) {
      ...
      ((kebabKey = hyphenate(key)) === key || !hasOwn2(rawProps, kebabKey))) {
        if (options) {
...
function setFullProps(instance, rawProps, props, attrs) {
  ...
  if (rawProps) {
    for (let key in rawProps) {
      ...
      const value = rawProps[key];
      let camelKey;
      if (options && hasOwn2(options, camelKey = camelize(key))) {
...
function validateProps(rawProps, props, instance) {
  ...
  for (const key in options) {
    ...
    if (opt == null)
      continue;
    validateProp(key, resolvedValues[key], opt, !hasOwn2(rawProps, key) && !hasOwn2(rawProps, hyphenate(key)));
  }
}
...
function _createVNode(type, props = null, children = null, patchFlag = 0, dynamicProps = null, isBlockNode = false) {
  ...
  const shapeFlag = isString(type) ? 1 : isSuspense(type) ? 128 : isTeleport(type) ? 64 : isObject2(type) ? 4 : isFunction2(type) ? 2 : 0;
